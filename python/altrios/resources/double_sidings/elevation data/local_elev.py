...
origin_elev_meters = 1300


print(f"\n=========== Results: Corridor {corridor_name} ===========")
...
USGS_with_both_filters_file  = f"/Users/qianqiantong/PycharmProjects/altrios-private/altrios/python/altrios/resources/double_sidings/elevation data/usgs_elevation_{corridor_name}_grade_data.xlsx"
USGS_with_Median_Filter_file = f"/Users/qianqiantong/PycharmProjects/altrios-private/altrios/python/altrios/resources/double_sidings/elevation data/USGS_no_sal_elevation_{corridor_name}_grade_data.xlsx"
USGS_with_Neither_Filter_file = f"/Users/qianqiantong/PycharmProjects/altrios-private/altrios/python/altrios/resources/double_sidings/elevation data/USGS_no_mean_elevation_{corridor_name}_grade_data.xlsx"
...
def find_dominant_peak(elev, prominence=20):
    ...
    if len(peaks) == 0:
        return np.argmax(elev)
    return peaks[np.argmax(props["prominences"])]


# ===============================================================
# Processing Function for All USGS Variants
# ===============================================================
def process_usgs_profile(usgs_dist, usgs_elev, label):
    # Clean
    ...
    if lag > 0:
        shifted = np.concatenate([np.full(lag, usgs_norm[0]), usgs_norm[:-lag]])
    elif lag < 0:
        shifted = np.concatenate([usgs_norm[-lag:], np.full(-lag, usgs_norm[-1])])
    else:
        shifted = usgs_norm.copy()

    # Vertical align
    v_offset = track_norm[track_peak] - shifted[track_peak]
    aligned = shifted + v_offset

    # Absolute elevation
    final = aligned + origin_elev_meters

    # Compute metrics
    up, down = compute_up_down(final)
    sign_match = elevation_to_sign(final)
    mse = np.mean((final - track_final)**2)
    mismatch = np.mean(track_sign != sign_match)

    print(f"{label} -> MSE = {mse:.2f}, Uphill={up[-1]:.1f}, Downhill={down[-1]:.1f}, Sign mismatch={mismatch*100:.2f}%")

    return final, mse, up[-1], down[-1], mismatch


# ===============================================================
# Load TRACK and BASE USGS
# ===============================================================
track_df = pd.read_excel(track_file)
USGS_with_both_filters_df  = pd.read_excel(USGS_with_both_filters_file)
usgs_nosal_df = pd.read_excel(USGS_with_Median_Filter_file)
usgs_nomean_df = pd.read_excel(USGS_with_Neither_Filter_file)
...
# Resample Track
max_common = min(track_dist.max(), USGS_with_both_filters_df["total_dist_meters"].max())
...
# ===============================================================
# Process all three USGS datasets
# ===============================================================
results = {}

for label, df in {
    "USGS_with_both_filters": USGS_with_both_filters_df,
    "USGS_with_Median_Filter": usgs_nosal_df,
    "USGS_with_Neither_Filter": usgs_nomean_df,
}.items():

    dist = df["total_dist_meters"].values
    elev = df["elevation_meters"].values

    final, mse, uph, downh, mismatch = process_usgs_profile(dist, elev, label)
    results[label] = {
        "final": final,
        "mse": mse,
        "uphill": uph,
...
plt.plot(common_x/1000, track_final, label="Track Chart", lw=1.8, color='black')
plt.plot(common_x/1000, results["USGS_with_both_filters"]["final"], label="USGS with Median and Savitsky-Golay filter")
plt.plot(common_x/1000, results["USGS_with_Median_Filter"]["final"], label="USGS with Median Filter")
plt.plot(common_x/1000, results["USGS_with_Neither_Filter"]["final"], label="USGS no Median or Savitsky-Golay filter")

plt.xlabel("Distance (km)")
plt.ylabel("Elevation (m)")
plt.title(f"Elevation Profile Comparison ({corridor_name})")
plt.legend()
plt.grid(True)
plt.tight_layout()
plt.show()
