"""Tests for :mod:`altrios.workflow_engine.loader`.
...
from altrios.workflow_engine import LoaderError, load_catalog, load_site
from altrios.workflow_engine.catalog import Catalog
from altrios.workflow_engine.yaml_loader import YamlLoaderError
...
from altrios.workflow_engine.tests import _loader_helpers  # noqa: F401
...
def test_load_catalog_with_python_module(tmp_path: Path):
    yaml = CATALOG_MINIMAL_YAML.replace(
        "name: test_catalog\n",
        "name: test_catalog\npython_module: altrios.workflow_engine.tests._loader_helpers\n",
    )
    p = _write(tmp_path / "catalog.yaml", yaml)
    cat = load_catalog(p)
    assert cat.python_module == "altrios.workflow_engine.tests._loader_helpers"
...
def test_resolve_catalog_reference_filesystem(tmp_path: Path):
    ...
    from altrios.workflow_engine.loader import _resolve_catalog_reference
