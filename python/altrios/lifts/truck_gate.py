...
def truck_entry(env, terminal, truck_obj, oc, train_schedule):
    ...
    ingate_request = terminal.state.in_gates.request()
...
def truck_arrival(env, terminal, train_schedule):
    ...
    terminal.state.total_oc[train_id] = oc_number

    oc_start = terminal.state.OC_COUNT[train_id]
    for oc_id in range(oc_start, oc_start + oc_number):
        terminal.state.oc_store.put(container(type='Outbound', id=oc_id, train_id=train_id))
    terminal.state.OC_COUNT[train_id] = oc_start + oc_number
    ...
    for _ in range(truck_entries_needed):
        ...
        oc = yield terminal.state.oc_store.get()
        env.process(truck_entry(env, terminal, this_truck, oc, train_schedule))
        yield terminal.state.truck_store.put(this_truck)
    ...
    if not terminal.state.all_trucks_arrived_events[train_id].triggered:
        terminal.state.all_trucks_arrived_events[train_id].succeed()


def truck_exit(env, terminal, truck_obj, ic, train_schedule):
    train_id = train_schedule['train_id']
    out_gate_request = terminal.state.out_gates.request()
